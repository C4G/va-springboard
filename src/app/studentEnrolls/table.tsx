...
export default function EnrollmentTable() {
  ...
  const filteredEnrollments = React.useMemo(() => {
    if (!id) return [];
    return enrollments.filter((g) => g.studentId === id);
  }, [enrollments, id]);
  ...
  const columns: ColumnDef<Enrollment>[] = [
    {
      ...
      header: 'Student',
      cell: ({ row }) => (
        <div>{`${students.find((s) => s.id === row.original.studentId)?.firstName} ${students.find((s) => s.id === row.original.studentId)?.lastName}`}</div>
      ),
    },
    {
